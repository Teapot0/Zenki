...
all_name = pd.read_excel('./data/all_stock_names.xlsx',index_col='Unnamed: 0')
...
close = pd.read_csv('./data/price/daily/close.csv', index_col='Unnamed: 0', date_parser=dateparse)
...
# ST
st_df = pd.read_csv('./data/price/is_st.csv', index_col='Unnamed: 0',date_parser=dateparse)
...
fund_sum_df['ratio_diff']= fund_sum_df['ratio'] - fund_sum_df['ratio_last']
zc_list = list(fund_sum_df[fund_sum_df['ratio_diff']>0].index) # 基金增仓
list2 = list(set.intersection(*map(set,[big_fund_hold_list, zc_list])))

rpsn1_3 = rps_n1_raw.loc[list2].sort_values(ascending=False)
rpsn2_3 = rps_n2_raw.loc[list2].sort_values(ascending=False)
rpsn3_3 = rps_n3_raw.loc[list2].sort_values(ascending=False)
rpsn4_3 = rps_n4_raw.loc[list2].sort_values(ascending=False)
rpsn5_3 = rps_n5_raw.loc[list2].sort_values(ascending=False)

rps_dict2 = {'rps20_name': all_name['short_name'].loc[rpsn1_3.index].values,
            'rps20': rpsn1_3.values,
            '基金持仓比例当季_20': fund_sum_df['ratio'].loc[rpsn1_3.index].values,
            '基金持仓比例上季_20': fund_sum_df['ratio_last'].loc[rpsn1_3.index].values,

            '基金股rps40': all_name['short_name'].loc[rpsn2_3.index].values,
            'rps40': rpsn2_3.values,
            '基金持仓比例当季_40': fund_sum_df['ratio'].loc[rpsn2_3.index].values,
            '基金持仓比例上季_40': fund_sum_df['ratio_last'].loc[rpsn2_3.index].values,

            '基金股rps60': all_name['short_name'].loc[rpsn3_3.index].values,
            'rps60': rpsn3_3.values,
            '基金持仓比例当季_60': fund_sum_df['ratio'].loc[rpsn3_3.index].values,
            '基金持仓比例上季_60': fund_sum_df['ratio_last'].loc[rpsn3_3.index].values,

            '基金股rps120': all_name['short_name'].loc[rpsn4_3.index].values,
            'rps120': rpsn4_3.values,
            '基金持仓比例当季_120': fund_sum_df['ratio'].loc[rpsn4_3.index].values,
            '基金持仓比例上季_120': fund_sum_df['ratio_last'].loc[rpsn4_3.index].values,

            '基金股rps250': all_name['short_name'].loc[rpsn5_3.index].values,
            'rps250': rpsn5_3.values,
            '基金持仓比例当季_250': fund_sum_df['ratio'].loc[rpsn5_3.index].values,
            '基金持仓比例上季_250': fund_sum_df['ratio_last'].loc[rpsn5_3.index].values}
rps_df2 = pd.DataFrame({key: pd.Series(value) for key, value in rps_dict2.items() })



# 单元格颜色
writer = pd.ExcelWriter('./data/rps_df_{}.xlsx'.format(today), engine='xlsxwriter')
rps_df.to_excel(writer, sheet_name='Sheet1')
rps_df2.to_excel(writer, sheet_name='基金当季持仓大于3%且当季增仓')
