...
start = close_5m.index[0].split(' ')[0]
end = close_5m.index[-1].split(' ')[0]
...
rv = (r2.rolling(48, min_periods=1).sum()).iloc[47::48,]
rv_pos = ((r2[close_5m_rts > 0]).rolling(48,min_periods=1).sum()).iloc[47::48,]
rv_neg = ((r2[close_5m_rts < 0]).rolling(48,min_periods=1).sum()).iloc[47::48,]

rsj = (rv_pos - rv_neg)/rv

factor = rsj
factor.index = [x.split(' ')[0] for x in factor.index]
# factor.to_csv('/Users/caichaohong/Desktop/Zenki/factors/5min_neg_rts_volstd.csv')
