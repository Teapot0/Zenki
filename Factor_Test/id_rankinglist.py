...
# IND_RANKINGLIST
ind_df = pd.read_csv('/Users/caichaohong/Desktop/Zenki/price/stock_swl1_industry.csv',index_col='Unnamed: 0')
ind_ret = pd.concat([daily_rts.stack(), ind_df.stack()],axis=1)
...
ret_rank = ind_ret.pivot(index='date', columns='stock', values='descending')
up = pd.DataFrame(0, index=ret_rank.index,columns=ret_rank.columns)
flag = pd.DataFrame(ret_rank.values<=10, index=ret_rank.index,columns=ret_rank.columns)
up[flag] = 1
