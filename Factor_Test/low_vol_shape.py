...
# vol_rts_5m = vol_5m.pct_change(1)
...
vol_rts = vol_daily.pct_change(1) * z_df

# low vol_shape
df_low = pd.DataFrame(daily_rts.values< daily_rts.rolling(60).mean().values, index=daily_rts.index, columns=daily_rts.columns)
df_vol = vol_rts[df_low]

factor = - df_vol.rolling(60,min_periods=1).mean()/ df_vol.rolling(60,min_periods=1).std()
