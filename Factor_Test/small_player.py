...
vol_5m = pd.read_csv('/Users/caichaohong/Desktop/Zenki/price/5m/vol_5m.csv',index_col='Unnamed: 0')
close_5m.index = [x.split(' ')[0] for x in close_5m.index]
vol_5m.index = [x.split(' ')[0] for x in close_5m.index]

vwap = pd.DataFrame(index=sorted(np.unique(close_5m.index)), columns=close_5m.columns)
for date in tqdm(vwap.index):
    vwap.loc[date] = (close_5m.loc[date] * vol_5m.loc[date]).sum() / vol_5m.loc[date].sum()

vwap = vwap.apply(lambda x: x.astype(float),axis=1)
net_amount_s = pd.read_csv('/Users/caichaohong/Desktop/Zenki/price/money_flow/net_amount_s.csv', index_col='Unnamed: 0')
net_amount_s = net_amount_s * 10**(4)
net_pct_s = pd.read_csv('/Users/caichaohong/Desktop/Zenki/price/money_flow/net_pct_s.csv', index_col='Unnamed: 0')
net_vol_s = net_pct_s * 0.01 * vol_daily

vwap_s = net_amount_s / net_vol_s
...
#  corr
factor = vwap_s.loc[vwap.index]/vwap -1
