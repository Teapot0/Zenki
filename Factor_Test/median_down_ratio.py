...
x = ((close_5m_rts[close_5m_rts<0]).fillna(0)**2).rolling(40*48).mean()
y = (close_5m_rts**2).rolling(40*48).mean()
temp_factor = (x/y).iloc[47::48]
temp_factor.index = [x.split(' ')[0] for x in temp_factor.index]
temp_factor = temp_factor.replace(np.inf,0)
factor = - abs(temp_factor.sub(temp_factor.mean(axis=1),axis=0))
