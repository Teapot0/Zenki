...
industry_diff_rts = daily_rts.copy(deep=True)

for ind in ind_list.index:
    tmp_stock = list(set(get_industry_stocks(ind, date=close_daily.index[-1])).intersection(set(close_daily.columns)))
    tmp_rts = daily_rts[tmp_stock].mean(axis=1)
    industry_diff_rts[tmp_stock] = industry_diff_rts[tmp_stock].sub(tmp_rts,axis=0)


factor = - industry_diff_rts.rolling(15).mean() / industry_diff_rts.rolling(15).std()
