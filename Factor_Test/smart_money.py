...
s1 = abs(close_1m_rts) / (np.sqrt(vol_1m * 10**(-4))) * 100

# factor
date_list = close_daily.index[(close_daily.index>='2021-01-01') & (close_daily.index<='2021-04-30')]

def smart_money_test(type):
    factor = pd.DataFrame(index= date_list, columns=close_1m.columns)

    for i in tqdm(range(2399,s1.shape[0], 240)):
        tmp_end = s1.index[i]
        tmp_date = tmp_end.split(' ')[0]
        tmp_df = s1.iloc[i-2399:i,:]
        temp_vol = vol_1m.iloc[i-2399:i,:]
        temp_close = close_1m.iloc[i-2399:i, :]
...
factor = smart_money_test(type='kurt')
