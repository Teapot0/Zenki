...
price_df['close_btc'] = 1/price_df['close'] # 每1张 对应多少btc
#btc收益率
price_df['rts_btc_1'] = price_df['close_btc'].shift(1) - price_df['close_btc']


#close收益率
price_df['rts_close_1'] = price_df['close'].pct_change(1)

# 乘100
initial_position = 100
price_df['close_btc'] = initial_position* price_df['close_btc']
price_df['rts_btc_1'] = initial_position* price_df['rts_btc_1']
...
price_df['buy_signal'] = ((price_df['rts_close_1'] > 0.001) & (price_df['vol_ex1_big'] == 1)) * 1
...
for i in tqdm(range(price_df.shape[0]-1)):
    if hold_status == 0: # 当天没有持仓
        ...
        tmp_rts = price_df['rts_btc_1'][i]  # btc收益

        tmp_net_value.append(tmp_net_value[-1] * (1 + price_df['rts_close_1'][i]))
...
price_df['hold_rts_btc'] = price_df['rts_btc_1'] * (1 - price_df['comm_fee']) * price_df['hold_status']
initial_margin = 0.1  # 保证金比例
price_df['margin_rate'] = 0.1  # 初始保证金比例
price_df['margin_rate'] = (initial_position*initial_margin + initial_position*price_df['hold_rts_btc'].cumsum()) / price_df['close_btc']
