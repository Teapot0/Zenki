...
save_path = './data/30m_data/'


for stock_id in tqdm(stock_ids[135:]):
    # st_tm = time.time()
    tmp_df = get_price(security=stock_id, start_date=start_date, end_date=end_date, frequency='30m',
                          fields=fields, fill_paused=False)
    tmp_df.to_parquet(save_path + 'df30m_{}.parquet'.format(stock_id))
