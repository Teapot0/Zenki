...
df = list(get_fundamentals(query(valuation.code).filter(valuation.market_cap > 100))['code'])  # 市值大于100的股票池
roe = pd.read_csv('/Users/caichaohong/Desktop/Zenki/financials/roe.csv', index_col='statDate')
rev_growth = pd.read_csv('/Users/caichaohong/Desktop/Zenki/financials/total_revenue_growth.csv', index_col='statDate')
np_growth = pd.read_csv('/Users/caichaohong/Desktop/Zenki/financials/net_profit_growth.csv', index_col='statDate')

# 73个
rev_growth_value = 10
np_growth_value = 10
roe_value = 2
stock_list = list(set(rev_growth.mean()[rev_growth.mean()>rev_growth_value].index).intersection(set(np_growth.mean()[np_growth.mean()>np_growth_value].index), set(roe.min()[roe.min()>roe_value].index), set(df)))
...
def kangdie_plot(start, end, stock_list):
    ...
    for j in tqdm(range(0, len(stock_list), 9)):  # 每9张图一个
        ...
        for i in range(j, min(j + 9, len(stock_list))):
            ...
            ax.plot(plot_df[stock_code], 'black')
            ax.plot(plot_df['hs300'], 'blue')
